        ...
        try:
            ...
            if expected is None:
                ...
                if is_correct and is_right_size:
                    ...
                    print(
                        f" {YELLOW}•{NC} {content:<{PAD_LENGTH}} [{GREEN} OK {NC}] (n={expected_len})"
                    )
                else:
                    print(f" {YELLOW}•{NC} {content:<{PAD_LENGTH}} [{RED}FAIL{NC}]")
                    if not is_right_size:
                        print(
                            f"    {RED}└── Cardinalidad incorrecta. Esperada: {expected_len}, Obtenida: {len(res)}{NC}"
                        )
                    if not is_correct:
                        print(
                            f"    {RED}└── Los subconjuntos no coinciden con lo esperado.{NC}"
                        )
